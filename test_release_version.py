...
class ReleaseVersionTests(unittest.TestCase):
    ...
    def test_workflows_enforce_stable_release_contract(self):
        ...
        self.assertIn("name: Verify state release compatibility", production)
        self.assertIn("steps.state.outputs.ready == 'true'", production)
        self.assertNotIn("force: true", production)
        self.assertIn("needs: tests", tests)
        self.assertIn("python live_smoke.py", tests)
        self.assertIn("name: Rebuild complete notice database", tests)
        self.assertIn("Rebuilt archive shrank", tests)
